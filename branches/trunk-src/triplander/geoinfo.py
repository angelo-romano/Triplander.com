...
from triplander.wikimarkup import parse as wikiparse
...
def yahoolocal_coordinates(city, country):
    ...
    # retrieve info
    yt_url = """http://local.yahooapis.com/MapsService/V1/geocode?appid=%s&city=%s""" % (YAHOO_DEVAPP_ID, urllib.quote(yt_query.encode('utf-8')))
    httpsched = get_http_scheduler()
    httpwhat  = httpsched.urlopen(service="yahootravel",url=yt_url,xml=True,alt_delta=2)
    ...
    for k in xmlobj.keys():
        if(len(xmlobj[k]) == 0): return None # verify
...
def gmaplocal_coordinates(city, country):
    ...
    # retrieve info
    yt_url = """http://maps.google.com/maps/geo?key=%s&q=%s&output=xml&hl=en-gb""" % (GOOGLEMAPS_APIKEY, urllib.quote(yt_query.encode('utf-8')))
    httpsched = get_http_scheduler()
    httpwhat  = httpsched.urlopen(service="yahootravel",url=yt_url,xml=True,alt_delta=2)
    
    # check if errors were retrieved
    
    if (httpwhat.getElementsByTagName('code')[0].childNodes[0].data != '200'): return None
    
    placemark = httpwhat.getElementsByTagName('Placemark')
    if(len(placemark)==0) : return None
    xmlset = []
    for p in placemark:
        xmlobj = {'ranking': 1}
        # check for accuracy
...
def delicious_popular(city_name):
    ...
    #httpresp = urllib2.urlopen(base_url)
    httpsched = get_http_scheduler()
...
def _open_panoramio_conn(coords,eps_factor=1,howmany=10):
    ...
    eps=eps_factor*0.016
    
    min_coordinates = (coords[0] - eps, coords[1] - eps)
    max_coordinates = (coords[0] + eps, coords[1] + eps)
    url = "http://www.panoramio.com/map/get_panoramas.php?order=popularity&set=public&size=square&from=0&to=%d&minx=%f&miny=%f&maxx=%f&maxy=%f" % (howmany, min_coordinates[1], min_coordinates[0], max_coordinates[1], max_coordinates[0])
    
    httpsched = get_http_scheduler()
    httpwhat  = httpsched.urlopen(service="panoramio",url=url,json=True)
    
    if(not httpwhat.has_key('photos')): return None
    return httpwhat
