...
_cleanup = [
		    (re.compile(ur'\[\[Image:(.+)\]\]\s*$', re.UNICODE | re.MULTILINE), ur''),
		    (re.compile(ur'\[\[Image:([^\]]+)\]\]', re.UNICODE | re.MULTILINE), ur''),
		    (re.compile(ur'\[\[File:(.+)\]\]\s*$', re.UNICODE | re.MULTILINE), ur''),
		    (re.compile(ur'\[\[File:([^\]]+)\]\]', re.UNICODE | re.MULTILINE), ur''),
		    (re.compile(ur'\[\[Category:([^\]]+)\]\]', re.UNICODE), ur''),
		    (re.compile(ur'<noinclude>(.*?)</noinclude>', re.UNICODE | re.MULTILINE), ur''),
		    (re.compile(ur'\[\[([a-z\-]+):([^\]]+)\]\]', re.UNICODE), ur''),
		    (re.compile(ur'\[\[([^\]\|]+)\]\]', re.UNICODE), ur'\1'),
		    (re.compile(ur'\[\[([^\]\|]+)\|([^\|]+)\]\]', re.UNICODE), ur'\2'),
		    (re.compile(ur'\[([^\]]+)\]', re.UNICODE), ur''),
		    #(re.compile(ur'\[\[', re.UNICODE | re.MULTILINE), ur'(jam)'),
		    (re.compile(ur'\s?\([^a-zA-Z0-9]+\)', re.UNICODE), ur''),
		    (re.compile(ur'{{[A|a]udio\|[^\|]+\|([^}]+)}}', re.UNICODE), ur'\1'),
		    (re.compile(ur'<ref[^>]*\s*/\s*>', re.UNICODE | re.MULTILINE), ur''),
		    (re.compile(ur'<ref[^>]*>(.*?)</ref>', re.UNICODE | re.MULTILINE), ur''),
		    (re.compile(ur'^[ ]+', re.UNICODE | re.MULTILINE), ur''),
		    #(re.compile(ur'^[ ]<+', re.UNICODE | re.MULTILINE), ur''),
		      ]
_post_cleanup = {
				 "replacements": [
				 ...
				 "regexps": [
					 #(re.compile(ur"(<b>[^<]+</b>) \([^\)]+\)", re.UNICODE), ur"\1"),
					 (re.compile(ur"^<dl>\s*<dd>\s*(.*?)\s*</dd>\s*</dl>\s*", re.UNICODE | re.MULTILINE), ur''),
					 (re.compile(ur"\([^a-zA-Z0-9][^\)]*\)\s*", re.UNICODE), ur''),
					 (re.compile(ur"[ ]{2,}", re.UNICODE), ur' '),
                     (re.compile(ur'\s?\(\s*pron[o]?unciation[^a-zA-Z]*\)', re.UNICODE | re.MULTILINE), ur''),
                     (re.compile(ur'\s?\(\s*pronounced[^a-zA-Z]*\)', re.UNICODE | re.MULTILINE), ur''),
					 (re.compile(ur"(<ref[^>]*>[^<]+</ref>)", re.UNICODE | re.MULTILINE), ur""),
					 (re.compile(ur"&lt;[^&]+&gt;.*?&lt;/[^&]+&gt;", re.UNICODE | re.MULTILINE), ur""),
				 ]
				}
...
class Parser(BaseParser):
	...
	def parse(self, text):
		...
		text = self.doBlockLevels(text, True)
		text = self.unstripNoWiki(text)
		text = self.removeCruft(text, True)
		text = text.split(u'\n')
		text = u'\n'.join(text)
		if taggedNewline and text[-1:] == u'\n':
			text = text[:-1]
	...
	def removeCruft(self, text, post=False):
		if (not post):
			what = ""
			for c in _cleanup:
				count = 1
				while(count > 0): 
					(text, count) = c[0].subn(c[1],text,re.MULTILINE)
			# remove templates
			tmpl_start = "{{"
			tmpl_end   = "}}"
			...
			tmpl_exit      = False
			tmpl_count     = 1
			#fh = codecs.open("ciccio.txt","w","utf8")
			while(not tmpl_exit):
				tmpl_start_idx = text.find(tmpl_start,tmpl_base_idx)
				tmpl_end_idx   = text.find(tmpl_end,tmpl_base_idx)
				tmpl_next_idx  = text.find(tmpl_start,tmpl_base_idx+len(tmpl_start))
				#s = "%d %d %d %d %d %s\n" % (tmpl_count,tmpl_base_idx,tmpl_start_idx,tmpl_end_idx,tmpl_next_idx, text[:256])
				#fh = codecs.open("ciccio.txt","w","utf8")
				#s2 = s.encode('utf8').decode('utf8')
				#fh.write(s2)
				if (tmpl_end_idx == -1 and tmpl_start_idx == -1):
					tmpl_exit = True
				elif (tmpl_end_idx == -1):
					text = text[0:tmpl_start_idx]
				elif (tmpl_start_idx == -1):
					text = ''
				else:
					if(tmpl_next_idx == -1 or (tmpl_next_idx > tmpl_end_idx)):
						if(tmpl_end_idx != -1):
							text = text[0:tmpl_start_idx]+text[tmpl_end_idx+len(tmpl_end):]
						tmpl_base_idx = 0					
					else:
						tmpl_base_idx = tmpl_next_idx
			...
			for c in _post_cleanup['replacements']:
				text = text.replace(c[0],c[1])
			for c in _post_cleanup['regexps']:
				text = c[0].sub(c[1],text,re.MULTILINE)
