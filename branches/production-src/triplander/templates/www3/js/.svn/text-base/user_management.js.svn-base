var CurrentUser = {
	username: BaseJSSettings.logged_user_name,
	user_id : BaseJSSettings.logged_user_id,
	username_box: null,
	password_box: null,
	login_box: null,
	username_account_span: null,
	init: function() {
		if (CurrentUser.username_box == null) CurrentUser.username_box = $('#login_username');
		if (CurrentUser.password_box == null) CurrentUser.password_box = $('#login_password');
		if (CurrentUser.login_box == null)    CurrentUser.login_box = $('#header_login_form');
		if (CurrentUser.username_account_span == null) CurrentUser.username_account_span = $('#header_login_account > span');
		CurrentUser.login_box.css('display','none');
		
		if (CurrentUser.username != '') {
			CurrentUser.username_account_span.html(CurrentUser.username);			
		}
	},
	loginUser: function() {
		var params = {
			username: CurrentUser.username_box.val(),
			password: MD5.hex_md5(CurrentUser.password_box.val())
		};

		$.post(BaseJSSettings.blueURL_user_login, params, function(json){
			if(json.error == 0) {
				window.location.reload(false); // ok, reload it
			} else {
	...
	logoutUser: function() {
		...
		$.post(BaseJSSettings.blueURL_user_logout, params, function(json){
			CurrentUser.username = '';
			
			window.location.reload(false); // get rid of unauthorized links
		});
	}
};
